...
*** runtime error for (cond) miss match

*** little syntax check for (quote) (partquote)

*** little syntax check for (case)

*** runtime error for (case) miss match
...
*** (case)
    ...
    (+jojo case/expend-if-then (-> :tags :answer -- sexp)
      (cond [:tags 'else eq?]
            ...
            [:tags cons?]
            `(if dup (quote (@ :tags .cdr)) {eq?} list-eqv?
                 then drop (@ :answer))

            else
            `(if dup .car (quote (@ :tags)) eq?
                 then drop (@ :answer))))
    #+end_src
