...
*** set-cmd-flag
    ...
    (define set-cmd-flag/loop
      (jojo
        (note index -> {set flag})
        (if dup cmd-number eq? then drop end)
        dup index->cmd-string
...
*** load-cmd-path
    ...
    (define load-cmd-path/loop
      (jojo
        ...
        (if dup cmd-number eq? then drop end)
        dup index->cmd-string
        (if dup string/flag? then drop inc (loop))
        (if dup path/readable? then path/load inc (loop))
        (el (cat "- load-cmd-path fall" newline
                 "  path : " string/print newline
                 "  cmd index : " int/print newline
                 "  is not readable" newline)
            debug)))

    (define load-cmd-path
      (jojo
        (int 1) load-cmd-path/loop))
    #+end_src
