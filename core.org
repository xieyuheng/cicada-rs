...
#+title: core
- fix functions about file
  - such as 'file/readable?'
- detailed report in 'load-module/get-path'
- deadly need debuger repl
- syntax for byte
  instead of string/last-byte
- <jojo> and combinators
  - syntax for currying
  - to mock a mocking bird
- [docs] basic docs about composition
- [emacs] fix space line in generated html
* todo
  ...
  - [testing] assert for embedded testing
    we should also be able to separate test out of the code
    to load the code faster
  - fix the roots of gc
...
***** load-module/get-path
      ...
      (define load-module/get-path
        (jojo
          ...
          (if (< user-path) file/readable? then (< user-path) end)
          (if (< system-path) file/readable? then (< system-path) end)
