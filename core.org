...
#+title: core
- integer => int
  str => text [with utf8-char]
  string [only as byte string]
- move functions about string and jo to c code
- cat-jo to replace jo/append
- depending-stack
- use
    ...
    (define mark-cons-area
      (jojo
        ...
        (if current-mark max-mark eq? then
            sweep-cons-area
            (integer 0) (address current-mark) set-cell)
        current-mark (integer 1) add (address current-mark) set-cell
...
*** string/member?
    ...
    (define string/member?
      (jojo
        (note byte[not 0] string -> true or false)
        (if dup get-byte (integer 0) eq? then 2drop false end)
        (if 2dup get-byte eq? then 2drop true end)
        (el inc (loop))))
    #+end_src
...
*** string/find-byte
    ...
    #+begin_src jojo
    (define string/find-byte/loop
      (jojo
        (note byte string begin-address -> [index true] or [false])
        (> begin-address)
        (if dup get-byte (integer 0) eq? then 2drop false end)
        (if 2dup get-byte eq? then swap drop (< begin-address) sub true end)
        (el inc (< begin-address) (loop))))

    (define string/find-byte
      (jojo
        (note byte string -> [index true] or [false])
        dup string/find-byte/loop))
    #+end_src
...
*** jo/find-byte
    ...
    (define jo/find-byte
      (jojo
        (note byte jo -> [index true] or [false])
        jo->string string/find-byte))
    #+end_src
...
*** jo/part
    ...
    (define jo/part
      (jojo
        (note index-begin index-end jo -> jo)))
    #+end_src
...
*** defining-stack
    ...
    (define defining-stack <bare-stack> (: new)
      (note of list of name))
    #+end_src

*** binding-filter-for-current-module
...
*** jo-filter-for-dependence
    ...
    (define jo-filter-for-dependence
      (jojo
        ...
        (el (> full-name)
            (< full-name) full-name->module
            square-bar jo/append
            current-module-version jo/append
            square-ket jo/append
            (jo .) jo/append
            (< full-name) full-name->name jo/append)))
...
* test
  ...
  (test bare-stack-0 (: empty?) dup dot dot newline
        bare-stack-0 (: push: (data (integer 1)))
