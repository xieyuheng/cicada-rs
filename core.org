...
*** list
    ...
    (defun one-list (: [io] -> [compile])
      read-jo
      (if dup (string "{") string->jo eq? ->
          drop (string "null") string->jo here
          (recur) (string "cons") string->jo here
          (loop))
      (if dup (string "}") string->jo eq? ->
          drop end)
      (else (string "i-lit") string->jo here
            here
            (string "cons") string->jo here
            (loop)))

    (defun list (: [io] -> [compile])
      read-jo
      (if dup (string ")") string->jo eq? -> drop end)
      (if dup (string "{") string->jo eq? ->
          drop
          (string "null") string->jo here
          one-list (loop))
      (else drop (loop)))
    #+end_src
...
*** test
    ...
    (run (list {a b})
         ...
         dup cdr car jo->string print-string
         dup cdr cdr jo->string print-string
         drop)
    #+end_src
