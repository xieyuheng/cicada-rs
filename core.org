...
*** use null-table instead of null
...
* repl
  ...
  (+jojo repl/loop
    ...
    read-sexp
    (cond [dup jo-bound? not]
          ["- jo is not bound : " string-write
           jo-write newline]
          else [null cons compile-jojo apply])
...
*** (run)
    ...
    (+jojo sexp-list-run compile-jojo apply)
...
*** (+module)
    ...
    (+jojo +module (-> :body --)
      ...
      {(if table-merge not then
           ...
           debug)}
      null swap list-foldr :import-table!
