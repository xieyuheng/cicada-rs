...
*** fix string-emit
    ...
    - if we do not scan jojo at each run_gc,
      macro can not leave gc data in function body.

*** note about emit-lit, jo-emit, jo-compile
...
*** sexp-list-partquote-compile -- depends on (list)
