...
#+title: core
* module/import-one should load modules
* (include)
...
*** [test] table processing in module system
    ...
    (test
      ...
      (+module hiya/0.0.2
        (import hi/0.0.2
          {'hi3 eq?} table-key-filter
          {'hi- swap jo-append} table-key-map
          {(if dup 'hi-hi3 eq? then drop 'hi)}
          table-key-map)
        (import ya/0.0.2
          {'ya3 eq?} table-key-filter
          {(if dup 'ya3 eq? then drop 'ya)}
          table-key-map)
        (export
          hiya)
...
*** [test] table processing in module system again
    ...
    (test
      ...
      (assert
        hiya/0.0.3:hiya "hiya" equal?)

      (assert
        (run (import hiya/0.0.3)
          hiya "hiya" equal?)))
    #+end_src
