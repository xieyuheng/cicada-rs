...
*** obj-unify & field-unify

    - x -
      unify should return how it fail.

    - k -
      with the help of goal-stack ?

    - x -
      maybe not
      unify can return error info
      and every function which call unify
      should check the return value

    - k -
      but this is just like the explicit control of frame-stack

    ------

    - pieter -
      While you may feel compelled to tell the caller //why//
      nothing happened ("timeout error!"),
      this is like talking to strangers about your private life.
      It's what you don't say that lets people respect you.

    - x -
      we should return a simple value to let the caller know
      the unification failed
      and let the caller do the report
