...
*** (+union)
    ...
    new_keyword (
        ...
        function (env, sexp_list)
        {
            ...
            assert (! null_p (rest_list));
            let next = car (rest_list);
            if (next === ':')
                rest_list === cdr (cdr (rest_list)); // over an arrow
            let union_vect = [];
    ...
    new_keyword (
        ...
        function (env, sexp_list)
        {
            ...
            for (let field_name of rest_vect) {
                if (field_name_p (field_name))
                    reversed_field_name_vect.push (field_name);
            }
            let type_den =
