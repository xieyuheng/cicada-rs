...
*** >< free-graph-cat-s
    ...
    #+begin_src cicada
    free-graph-cat-s : -- graph-s -> category-s
    free-graph-cat-s = lambda
      let graph in instance
      object-t = lambda graph.node-t
      arrow-t = lambda graph.path-t
      arrow-eqv-t = lambda graph.path-eqv-t
      ...
      compose = lambda graph.link-path-c

      identity-left :
        -- a b graph.path-t % f
        -> a graph.node-path-c f graph.link-path-c
           f graph.path-eqv-t
      identity-left = lambda graph.node-left-path-eqv-c

      identity-right :
        -- a b graph.path-t % f
        -> f b graph.node-path-c graph.link-path-c
           f graph.path-eqv-t
      identity-right = lambda graph.node-rifht-path-eqv-c

      compose-associative :
        -- a b graph.path-t % f
           b c graph.path-t % g
           c d graph.path-t % h
        -> f g h graph.link-path-c graph.link-path-c
           f g graph.link-path-c h graph.link-path-c
           graph.path-eqv-t
      compose-associative = lambda
        let f g h in
        ><><><
