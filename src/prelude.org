...
*** nat-add-associative
    ...
    nat-add-associative x y z =
      case x
        zero-t eqv-c
        succ-t (eqv-apply succ-c (nat-add-associative x.prev y z))
    #+end_src
...
*** nat-add-succ-commutative-1
    ...
    nat-add-succ-commutative-1 x y =
      case x
        zero-t eqv-c
        succ-t (eqv-apply succ-c (nat-add-succ-commutative-1 x.prev y))
    #+end_src
...
*** nat-add-succ-commutative-2
    ...
    nat-add-succ-commutative-2 x y =
      case x
        zero-t eqv-c
        succ-t (eqv-apply succ-c (nat-add-succ-commutative-2 x.prev y))
    #+end_src
    ...
    list-remove-first x list =
      case list
        ...
        cons-t
          if (eq-p list.car x)
          then list.cdr
          else (cons-c list.car (list-remove-first list.cdr x))
    #+end_src
...
*** vect-append
    ...
    vect-append ante succ =
      case ante
        null-vect-t succ
        cons-vect-t (cons-vect-c ante.car (vect-append ante.cdr succ))
    #+end_src
...
*** vect-map
    ...
    vect-map fun list =
      case list
        null-vect-t list
        cons-vect-t (cons-vect-c (fun list.car) (vect-map fun list.cdr))
    #+end_src
    ...
    nat-order-category = category-cc
      ...
      identity-neutral-left x =
        case x
          zero-lteq-t eqv-c
          succ-lteq-t
            (eqv-apply succ-lteq-c (identity-neutral-left x.prev))

      identity-neutral-righ x =
        case x
          zero-lteq-t eqv-c
          succ-lteq-t
            (eqv-apply succ-lteq-c (identity-neutral-righ x.prev))

      compose-associative f g h =
        case [f g h]
          [zero-lteq-t _ _] eqv-c
          [succ-lteq-t succ-lteq-t succ-lteq-t]
            (eqv-apply succ-lteq-c (compose-associative f.prev g.prev h.prev))
    #+end_src
    ...
    list-container = container-cc
      ...
      map fun list =
        case list
          null-t null-c
          cons-t
            (cons-c (fun list.car) (map fun list.cdr))
    #+end_src
...
*** monad-tt.compose
    ...
    monad-tt.compose :
      -- (-- a -> (fun-t b))
         (-- b -> (fun-t c))
      -> (-- a -> (fun-t c))
    monad-tt.compose f g = (lambda [a] (bind (f a) g))
    #+end_src
...
*** tree-zip
    ...
    tree-zip x y =
      case [x y]
        ...
        [branch-t branch-t]
          do left <- (tree-zip x.left y.left)
    ...
    tree-numbering tree =
      case tree
        leaf-t lambda [n]
          (* (nat-inc n) (leaf-c n))
        branch-t
          do left <- (tree-numbering tree.left)
