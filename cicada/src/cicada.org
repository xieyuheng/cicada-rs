...
#+title: cicada
- no prelude -- use examples as docs and tests
  - use .md instead of //// line
    if we were to do so
    .md file must also be fully edit-able online
...
***** () and {}
      ...
      - the rule about when we can use () instead of {}
        and how we should use ()
        is not very good

        it is not purely a syntax sugar
        it depends on semantic and context

        currently the rule is that () will start fulfilling
        the first not yet fulfilled typed-var

      - in new version of the implementation
        I will use () purely as syntax sugar of {} based on context
